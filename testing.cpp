...
// NOTE - testData_t - чтоб знать что это тип

bool OneTest (testData_t test) {
    ...
    switch (numRoots) {
        case ONE:
            if (!isEqual(x1, test.x1ref)) {
                mistakes++; //NOTE - в остальных также сделать
            }
            break;
        case TWO:
        ...
        case NO:
            ...
            break;
        case INF:
            if (!(isnan(x1) && isnan(x2))) { // ALIFFKA_BEST_OF_THE_BEST - на вкус: мэйби другие корни чтоб различать
                mistakes++;
            }
            break;
        default:;
...
size_t allTests (void) {
// ХУЙНЯ - выравнивание
    testData_t TASKS[] = {{.a= 1,   .b = -3,      .c = 2,        .x1ref = 1,   .x2ref = 2,   .numRootsRef = 2},
    ...
    size_t howMuchTasks = sizeof(TASKS) / sizeof(TASKS[0]);  // ХУЙНЯ - naming
// NOTE - не int а size_t
    for (size_t i = 0; i < howMuchTasks; i++) {
        if (OneTest(TASKS[i])) {
            return i;
        }
    }
